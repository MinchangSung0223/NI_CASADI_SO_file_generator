...
{
#ifdef _WIN32
    // Windows 환경에서의 라이브러리 로드
    HMODULE handle = LoadLibrary(TEXT("libcasadi_win.dll")); // 공유 라이브러리 경로 설정
    if (!handle)
    {
    ...
    if (!GravityForces)
    {
        std::cerr << "Failed to load GravityForces function." << std::endl;
        FreeLibrary(handle);
        return EXIT_FAILURE;
    }

    // GravityForces_work 함수 포인터 가져오기
